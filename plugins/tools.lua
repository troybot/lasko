...
if matches[1] == 'عراب غادر' and is_sudo(msg) then
tdcli.sendMessage(msg.to.id, msg.id, 1, 'حسنا سيدي المطور 🚶🏻', 1, 'html')
tdcli.changeChatMemberStatus(msg.to.id, our_id, 'Left', dl_cb, nil)
botrem(msg)

end   

if matches[1] == " صلاحيات البوت " and not matches[2] and is_owner(msg) then
...
if secchk then
return tdcli.sendMessage(msg.to.id, msg.id, 1, 'البوت ليس ادمن في المجموعة ❌', 1, "md")
else
return tdcli.sendMessage(msg.to.id, msg.id, 1, 'البوت ادمن في المجموعة 👮🏻✅', 1, "md")
end
end, nil)
end
...
if is_sudo(msg) then rank = 'مطور البوت 💀'
elseif is_owner(msg) then rank = 'مدير المجموعه 👮🏻'
elseif is_sudo(msg) then rank = 'ادمن في البوت 👮🏻'
...
◈￤ﮧ`➖➖➖➖➖`
◈￤`امر 1` ⇜ اوامر الاداره  
◈￤`امر 2` ⇜ اوامر اعدادت الكروب
◈￤`امر 3` ⇜ اوامر الحماية 
◈￤`امر 4` ⇜ الاوامـر الـ؏ـامه 
◈￤`امر 5` ⇜ اوامر اضافه ردود
◈￤`امر 6` ⇜ اوامر الزخرفه    
◈￤`م المطور` ⇜ للمطور فقط 
◈￤ﮧ`➖➖➖➖➖`
◈￤`الـمـطـور`  ⇜  ]]..sudouser
